...
public class IndustriaDailies {
    ...
    public void onPlayerInteractEvent(PlayerInteractEvent.EntityInteractSpecific event) {
        ...
        if (!manager.playerSetQuests.containsKey(player.getStringUUID())) {
            return;
        }
        ...
        if (target instanceof EasyNPC<?> npc) {
            ...
            for (int i = 0; i < quests.size(); i++) {
                ...
                if ((quests.get(i).getTalkTo().isEmpty() || quests.get(i).getTalkTo() == null) && (quests.get(i).getHandIn().isEmpty() || quests.get(i).getHandIn() == null)) {
                    return;
                }
                ...
                if (quests.get(i).checkHandIn(target.getUUID().toString())) {
                    if (!quests.get(i).getHandIn().isEmpty()) {
                        if (quests.get(i).hasTalkedTo) {
                            if (manager.fullQuestCheckComplete(player, quests.get(i), target)) {
                                Utils.displayTitle(player, "Quest Completed!", ChatFormatting.GOLD);
                            }
